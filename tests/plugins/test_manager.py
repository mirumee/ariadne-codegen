...
def test_get_comment_calls_plugins_get_comment(plugin_manager_with_mocked_plugins):
    plugin_manager_with_mocked_plugins.get_comment(
        "comment", code="class Xyz:\n    pass", source="queries.graphql"
    )

    plugin1, plugin2 = plugin_manager_with_mocked_plugins.plugins
    assert plugin1.get_comment.called
    assert plugin2.get_comment.called
