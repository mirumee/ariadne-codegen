...
def test_execute_passes_kwargs_to_json_post(mocker):
    http_client = mocker.MagicMock()

    BaseClient(url="http://base_url", http_client=http_client).execute(
...
def test_execute_sends_json_request_with_headers_from_passed_kwargs(httpx_mock):
    httpx_mock.add_response()
    client = BaseClient(
        url="http://base_url",
        headers={
...
def test_execute_passes_kwargs_to_multipart_post(mocker, txt_file):
    http_client = mocker.MagicMock()
